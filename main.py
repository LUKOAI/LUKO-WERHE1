import customtkinter as ctk
from pathlib import Path
from datetime import datetime
from reportlab.lib.pagesizes import A4
from reportlab.pdfgen import canvas


def make_demo_pdf():
    out = Path("output")
    out.mkdir(exist_ok=True)
    p = out / f"demo_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    c = canvas.Canvas(str(p), pagesize=A4)
    c.setFont("Helvetica-Bold", 16)
    c.drawString(72, 800, "WERHE/WERKON - DEMO dokumentu")
    c.setFont("Helvetica", 11)
    c.drawString(72, 770, "Symulacja bez API Apilo")
    c.drawString(72, 750, "Data: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    c.drawString(72, 730, "Zamowienie: DEMO-0001")
    c.drawString(72, 710, "Kraj: USA, Kurier: UPS")
    c.drawString(72, 690, "Status: Delivered (symulacja)")
    c.save()
    return p


class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("WERHE/WERKON DEMO")
        self.geometry("760x420")
        ctk.set_appearance_mode("light")
        ctk.set_default_color_theme("blue")

        ctk.CTkLabel(self, text="Demo bez API (TeamViewer)", font=("Arial", 18, "bold")).pack(pady=16)
        ctk.CTkButton(self, text="Wygeneruj przykladowy PDF", command=self.run_demo).pack(pady=10)
        self.log = ctk.CTkTextbox(self, width=700, height=260)
        self.log.pack(pady=10)

    def run_demo(self):
        p = make_demo_pdf()
        self.log.insert("end", f"OK: wygenerowano {p}\n")
        self.log.see("end")


if __name__ == "__main__":
    App().mainloop()
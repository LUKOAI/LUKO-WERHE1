...
def generate_order_pdf(order: OrderRecord, screenshot_path: Path | None, output_path: Path, company_name: str) -> Path:
    ...
    lines = [
        ...
        f"Numer faktury: {order.invoice_number or 'brak danych'}",
        ...
        f"Typ realizacji: {order.warehouse_type.upper()}",
    ]
    for line in lines:
    ...
    if screenshot_path and screenshot_path.exists():
        ...
        c.setFont(FONT, 10)
        c.drawString(20 * mm, y - 12 * mm, "Brak screenshota trackingu - tracking URL niedostępny w danych API.")
        if order.tracking_number:
            c.drawString(20 * mm, y - 18 * mm, f"Numer przesyłki: {order.tracking_number}")
        if order.tracking_url:
            c.drawString(20 * mm, y - 24 * mm, f"URL: {order.tracking_url}")
...
def generate_summary_pdf(
    ...
    def draw_table(title: str, orders: list[OrderRecord], y_pos: float) -> float:
        ...
        c.drawString(55 * mm, y_pos, "Nr Amazon")
        c.drawString(95 * mm, y_pos, "Data")
        c.drawString(115 * mm, y_pos, "Kraj")
        c.drawString(130 * mm, y_pos, "Kwota")
        c.drawString(160 * mm, y_pos, "Faktura")
        y_pos -= 4 * mm
        ...
        c.setFont(FONT, 7)
        for order in orders:
            c.drawString(20 * mm, y_pos, order.order_number[:20])
            c.drawString(55 * mm, y_pos, (order.amazon_order_number or "-")[:22])
            c.drawString(95 * mm, y_pos, order.order_date.strftime("%Y-%m-%d"))
            c.drawString(115 * mm, y_pos, order.country_code)
            c.drawString(130 * mm, y_pos, f"{order.total_gross:.2f} {order.currency}")
            c.drawString(160 * mm, y_pos, (order.invoice_number or "-")[:22])
            y_pos -= 4 * mm
            if y_pos < 20 * mm:
                c.showPage()
                y_pos = height - 20 * mm
